...
public class AnalyzerShadowCopyTests {
    ...
    public void WithShadowCopiedAnalyzerReferences_ShouldKeepOriginalAssemblyWritable() {
        ...
        var nestedArtifactPath = Path.Combine(Path.GetDirectoryName(analyzerPath)!, "runtimes", "win-x64", "native", "helper.json");
        Directory.CreateDirectory(Path.GetDirectoryName(nestedArtifactPath)!);
        File.WriteAllText(nestedArtifactPath, "{ \"name\": \"helper\" }");
        ...
        Assert.That(File.Exists(Path.Combine(Path.GetDirectoryName(analyzerReference.FullPath)!, Path.GetRelativePath(Path.GetDirectoryName(analyzerPath)!, nestedArtifactPath))), Is.True);
